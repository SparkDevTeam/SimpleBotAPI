...
// Code for bot messages starts here
...
if($input!=NULL && $input!=""){
	if(contains($input,$greetings)){
		// If the input contains any of the greeting messages then greet the user.
		echo "Hello there, I am SparkBot. Nice to meet you :D";
	}else if(containsAll($input,array("tell","joke"))){
		// If the input contains 'tell' and 'joke' then tell a joke.
		// TODO: Add jokes :P
	}else{
		// If the input couldn't be matched with a reply then:
		echo "Sorry, I could not quite get what you meant there :(";
	}
}else{
	// If the input was empty then:
	echo "Sorry, I could not quite get what you meant there :(";
}

// Code for bot messages stops here

function contains($str, array $arr)	// Checks if the String $str contains any of the values in Array $arr
{
    foreach($arr as $a) {
...
function containsAll($str, array $arr)	// Checks if the String $str contains all the values in Array $arr
{
    foreach($arr as $a) {
